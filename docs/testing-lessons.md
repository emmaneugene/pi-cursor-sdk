...
This document records maintainer testing lessons for `pi-cursor-sdk`. It complements unit tests and the [Cursor live smoke checklist](./live-smoke-checklist.md). Use it when adding regression coverage, debugging false-green releases, or building isolated smoke harnesses.

For a **minimal one-session dogfood pass** (baseline env, one native + one bridge call, JSONL ID patterns, bootstrap manifest, edit diff card), use the [Cursor dogfood checklist](./dogfood-checklist.md) as focused evidence before the full release evidence bar.
...
- `test/native-replay-stress.test.ts` — plan strip → resync → grep replay; inactive-tool trace fallbacks
- `test/provider-replay-live-run.test.ts` — inactive replay tools emit trace instead of broken `toolUse`
- `test/native-replay-trace.test.ts` — shared inactive replay trace formatting
- `test/native-replay-routing.test.ts` — `resolveNativeReplayDisposition` and `partitionNativeToolsByActiveContext`
...
- Nested children disable native replay wrappers. They dispose their isolated SDK agent after the provider run completes.

Regression coverage:

- `test/extension-factory-guard.test.ts` — owner tokens reject stale release and release for every Pi shutdown reason
...
- `test/session-agent.test.ts` — a nested scope acquires its own SDK agent while the parent scope is busy
- `test/pi-tool-bridge.test.ts` — nested bridge shutdown does not reject a pending owner bridge call
- `test/provider-run-finalizer.test.ts` — live provider completion runs the nested lifecycle disposal path
...
If resync runs but `context.tools` is still stale (e.g. only `read` listed), the provider must **not** emit `toolUse` for inactive tools. `test/native-replay-stress.test.ts` covers that stale-snapshot path.
...
Example seed step used by `scripts/isolated-smoke.sh`:
...
Successful tool results are ignored even when file contents mention those strings (for example a `read` of `docs/testing-lessons.md` during plan-strip smoke).
...
- Distinguish published SDK `TokenUsage` from observed raw local `turn-ended.usage`: installed `@cursor/sdk` `toTokenUsage` sets `totalTokens = input+output+cacheRead+cacheWrite`, but captured local raw `turn-ended.usage` keeps `inputTokens` as the full prompt while cache fields partition it (see `test/fixtures/sdk-turn-ended-usage-1.0.23.json` / issue #196). Map raw turn-ended samples to pi as uncached `input = inputTokens - cacheReadTokens - cacheWriteTokens` and `totalTokens = inputTokens + outputTokens`.
- No single assistant message should persist SDK/full-agent-context-sized usage outside the selected model window.
- Real bad-session evidence should be reduced to a sanitized fixture, like `test/fixtures/run-usage-compaction-poison.jsonl`, instead of committing raw session JSONL.
...
Then use the [Cursor live smoke checklist](./live-smoke-checklist.md) for focused surfaces the scripts do not cover (bridge MCP, abort/cancel, full TUI observation, packaging review, cleanup).
...
| Whether the listed names are `pi__*` bridge MCP, Cursor-native (`browser_navigate`, `WebSearch`), or `cursor-replay-*` replay IDs | Three different surfaces (see [Cursor native tool replay](./native-tool-replay.md#live-bridge-vs-replay)) |
...
4. **No `toolCall` blocks, no error toast, user expected real execution** — check whether names are replay-only (`cursor-replay-*`) or Cursor-native MCP; replay never re-runs work ([replay doc](./native-tool-replay.md)).
...
- [Cursor live smoke checklist](./live-smoke-checklist.md)
- [Cursor native tool replay](./native-tool-replay.md)
- `scripts/isolated-smoke.sh`
...
- `test/helpers/provider-harness.ts` — Cursor SDK provider mocks and stream helpers (re-exports pi-harness fixtures; `createNativeToolDisplayPiForTest` for native replay)
