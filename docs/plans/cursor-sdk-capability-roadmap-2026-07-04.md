...
| Cursor bundled ripgrep initialization | **Implemented** | `src/cursor-sdk-platform-package.ts` resolves the installed SDK platform package before local agent creation; `test/cursor-sdk-platform-package.test.ts` covers direct and nested npm layouts plus override preservation. |
| Cursor bundled tree-sitter vendor initialization | **Implemented** | The same module pins `CURSOR_TREE_SITTER_VENDOR_DIR` before local agent creation so a default `pi` launch finds `vendor/tree-sitter` in the platform package instead of walking from the pi binary. |
| Cursor bundled cursorsandbox initialization | **Implemented** | Local `Agent.create` / `Agent.resume` temporarily set `process.argv[1]` to the platform package so the SDK locator finds `bin/cursorsandbox`. The SDK has no path env var for this helper. |
...
| Optional Cursor question prompt | **Removed in 0.5.0** | The dedicated `cursor_ask_question` tool and `PI_CURSOR_ASK_QUESTION` duplicated pi's richer bridged `AskUserQuestion` tool, so 0.5.0 removed them. |
