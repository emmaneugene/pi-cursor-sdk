...
The default/opt-out lane verifies the built-in local resume default resumes, then verifies `local.resume: false` opts out and creates a new agent while bootstrapping the transcript.
...
| `cursor-local-resume-default-dry-run` | `npm run smoke:local-resume:default-dry-run` | built-in default resumes and `local.resume: false` opts out |
...
Provider debug artifacts are required for every live suite through isolated `cursor-sdk.json` `debug.sdkEvents.enabled` and suite-scoped debug dirs.
