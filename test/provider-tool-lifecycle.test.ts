...
import { CursorSdkEventDebugSink } from "../src/sdk-event-debug.js";
import { installCursorSdkEventDebugUserConfig } from "./helpers/sdk-event-debug.js";
import { __testUtils as sessionAgentTestUtils } from "../src/session-agent.js";
import { CURSOR_TOOL_LIFECYCLE_DEFER_MS } from "../src/tool-lifecycle.js";
...
describe("streamCursor Cursor tool lifecycle", () => {
	...
	it("does not append deferred lifecycle progress after non-live run.wait resolves before slow debug capture", async () => {
		const restore = installCursorSdkEventDebugUserConfig({ enabled: true }, { runDir: "/tmp/pi-cursor-sdk-lifecycle-wait-finished" });
	...
	it("does not append deferred lifecycle progress after live background run.wait resolves before slow debug capture", async () => {
		...
		// The started-only shell call is suppressed on a successful text-producing
		// run, so the turn finishes directly with the final text and no card.
