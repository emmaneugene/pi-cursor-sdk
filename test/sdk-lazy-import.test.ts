...
function isAllowedCursorSdkDynamicImport(relativePath: string, specifier: string): boolean {
	return (
		(relativePath.endsWith("src/sdk-runtime.ts") && specifier === "@cursor/sdk")
		|| (relativePath.endsWith("src/session-store.ts") && specifier === "@cursor/sdk/sqlite")
	);
}
	...
	for (const path of paths) {
		...
		const visit = (node: ts.Node): void => {
			if (ts.isImportDeclaration(node) && importHasRuntimeBindings(node, false)) {
				...
				if (specifier?.startsWith("@modelcontextprotocol/sdk/") && !relativePath.endsWith("src/pi-tool-bridge-run.ts")) {
					offenders.push(`${relativePath}:${source.getLineAndCharacterOfPosition(node.getStart()).line + 1}: runtime import ${specifier}`);
				}
				if (specifier === "./pi-tool-bridge-run.js") {
...
function collectUnsafeHostPeerLoads(
	...
	nativeModuleLoaderPath: string = join(process.cwd(), "src", "sdk-platform-package.ts"),
): string[] {
		...
		const visit = (node: ts.Node): void => {
			...
			if (
				...
				const message = !isCanonicalNativeModule
					? `native module loader ${node.text} outside src/sdk-platform-package.ts`
					: !isCanonicalCreateRequireImport(node)
						? 'src/sdk-platform-package.ts must use import { createRequire } from "node:module"'
						: undefined;
				if (message) {
...
describe("Cursor SDK lazy runtime imports", () => {
	...
	it("fails closed for Pi package prefixes, CommonJS loaders, nested shared modules, and unresolved edges", () => {
		...
		const findings = collectUnsafeHostPeerLoads(runtimeModuleFiles(srcDir, sharedDir)).join("\n");
		expect(findings).toContain("native module loader node:module outside src/sdk-platform-package.ts");
		expect(findings).toContain("native module loader module outside src/sdk-platform-package.ts");
	...
	it("loads the installed SDK checkpoint store without the old root sqlite dependency", async () => {
		...
		const { loadCursorSdk } = await import("../src/sdk-runtime.js");
