...
import { resolveCursorSdkConfig, type CursorResolvedSdkConfig } from "../src/config.js";
...
vi.mock("../src/provider-turn-prepare.js", async (importOriginal) => {
	const actual = await importOriginal<typeof import("../src/provider-turn-prepare.js")>();
	return {
		...actual,
...
vi.mock("../src/provider-live-run-drain.js", async (importOriginal) => {
	const actual = await importOriginal<typeof import("../src/provider-live-run-drain.js")>();
	return {
		...actual,
...
describe("CursorProviderTurnRunner config snapshotting (F3)", () => {
	it("resolves the effective config exactly once per turn and threads the same snapshot into prepare, even if config changes during drain", async () => {
		const { CursorProviderTurnRunner } = await import("../src/provider-turn-runner.js");
